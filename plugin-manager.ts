...
const CONFIG_PATH = path.join(process.cwd(), 'harvest.config.ts');

async function readConfig(): Promise<HarvestConfig | null> {
  try {
...
async function writeConfig(config: HarvestConfig): Promise<void> {
  try {
