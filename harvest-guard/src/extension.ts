...
import { scan, type Severity } from 'harvest-scan';
  ...
  ctx.subscriptions.push(
    ...
    vscode.commands.registerCommand('harvest-guard.scanWorkspace', async () => {
      ...
      vscode.window.withProgress(
        { location: vscode.ProgressLocation.Notification, title: 'Harvest-Guard scan' },
        async () => {
          const res = await scan(ws.uri.fsPath, { mode: 'complete', stream: false });
          vscode.window.showInformationMessage(`Harvest-Guard score: ${res.score} (${res.rating})`);
        });
    })
  );
}
